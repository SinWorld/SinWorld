...
public interface BookDao {
	...
	public Integer totalCartByBookId(@Param("bookId")Integer bookId,@Param("userId")Integer userId);
	
	//删除购物项(逻辑删除)
	public void deleteCartByBookId(@Param("bookId")Integer bookId,@Param("flag") boolean flag);
	
	//批量删除购物项(逻辑删除)
	public void batchDeleteCart(@Param("userId")Integer userId,@Param("bookId")Integer bookId,@Param("flag") boolean flag);
	
	//购买商品后新增数据至收货地址表中
	public void addHMP_ADDRESS(HMP_Address hmp_address);
	
	//购买商品后生成支付订单
	public void addHMP_Form_Message(HMP_Form_Message message);
	...
	//我的收藏
	public List<BookInforMation>myCollection(@Param("userId")Integer userId,@Param("flag")boolean flag);
